...
class TestInterface(unittest.TestCase):
	def get_test_msg_class(self):
		return MessageFactory({
			...
			'format': ['id', 'x', 'y', 'direction'],
			'byteformat': '!IIIB'
		})

	def get_test_msg(self, x=1, y=3.33, direction=2):
		msg = self.get_test_msg_class()
		msg['x'] = x
		msg['y'] = y
	...
	def test_message_factory(self):
		TestMessage = self.get_test_msg_class()
		self.assertEqual(TestMessage.__name__, 'TestMessage')
	...
	def test_packing(self):
		...
		self.assertEqual(packed[8:9], struct.pack('!B', get_test_msg_class().enum_lookup('direction', 'south')))
